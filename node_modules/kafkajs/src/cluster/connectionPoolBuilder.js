...
module.exports = ({
  ...
  const getBrokers = async () => {
    ...
    list = list.filter(broker => broker && typeof broker === 'string' && broker.length > 0)
    list = brokers
    if (!brokers.length || !isValidBroker(brokers)) {
      list = [ 'localhost:9092' ]
      brokers = list  // Default broker if none specified
    }
